/*
Copyright 2024 The Kubernetes Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/
// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "github.com/intel/cloud-resource-scheduling-and-isolation/pkg/api/diskio/v1alpha1"
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeNodeDiskIOStatses implements NodeDiskIOStatsInterface
type FakeNodeDiskIOStatses struct {
	Fake *FakeDiskioV1alpha1
	ns   string
}

var nodediskiostatsesResource = v1alpha1.SchemeGroupVersion.WithResource("nodediskiostats")

var nodediskiostatsesKind = v1alpha1.SchemeGroupVersion.WithKind("NodeDiskIOStats")

// Get takes name of the nodeDiskIOStats, and returns the corresponding nodeDiskIOStats object, and an error if there is any.
func (c *FakeNodeDiskIOStatses) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.NodeDiskIOStats, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(nodediskiostatsesResource, c.ns, name), &v1alpha1.NodeDiskIOStats{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.NodeDiskIOStats), err
}

// List takes label and field selectors, and returns the list of NodeDiskIOStatses that match those selectors.
func (c *FakeNodeDiskIOStatses) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.NodeDiskIOStatsList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(nodediskiostatsesResource, nodediskiostatsesKind, c.ns, opts), &v1alpha1.NodeDiskIOStatsList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.NodeDiskIOStatsList{ListMeta: obj.(*v1alpha1.NodeDiskIOStatsList).ListMeta}
	for _, item := range obj.(*v1alpha1.NodeDiskIOStatsList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested nodeDiskIOStatses.
func (c *FakeNodeDiskIOStatses) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(nodediskiostatsesResource, c.ns, opts))

}

// Create takes the representation of a nodeDiskIOStats and creates it.  Returns the server's representation of the nodeDiskIOStats, and an error, if there is any.
func (c *FakeNodeDiskIOStatses) Create(ctx context.Context, nodeDiskIOStats *v1alpha1.NodeDiskIOStats, opts v1.CreateOptions) (result *v1alpha1.NodeDiskIOStats, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(nodediskiostatsesResource, c.ns, nodeDiskIOStats), &v1alpha1.NodeDiskIOStats{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.NodeDiskIOStats), err
}

// Update takes the representation of a nodeDiskIOStats and updates it. Returns the server's representation of the nodeDiskIOStats, and an error, if there is any.
func (c *FakeNodeDiskIOStatses) Update(ctx context.Context, nodeDiskIOStats *v1alpha1.NodeDiskIOStats, opts v1.UpdateOptions) (result *v1alpha1.NodeDiskIOStats, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(nodediskiostatsesResource, c.ns, nodeDiskIOStats), &v1alpha1.NodeDiskIOStats{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.NodeDiskIOStats), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeNodeDiskIOStatses) UpdateStatus(ctx context.Context, nodeDiskIOStats *v1alpha1.NodeDiskIOStats, opts v1.UpdateOptions) (*v1alpha1.NodeDiskIOStats, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(nodediskiostatsesResource, "status", c.ns, nodeDiskIOStats), &v1alpha1.NodeDiskIOStats{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.NodeDiskIOStats), err
}

// Delete takes name of the nodeDiskIOStats and deletes it. Returns an error if one occurs.
func (c *FakeNodeDiskIOStatses) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteActionWithOptions(nodediskiostatsesResource, c.ns, name, opts), &v1alpha1.NodeDiskIOStats{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeNodeDiskIOStatses) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(nodediskiostatsesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.NodeDiskIOStatsList{})
	return err
}

// Patch applies the patch and returns the patched nodeDiskIOStats.
func (c *FakeNodeDiskIOStatses) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.NodeDiskIOStats, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(nodediskiostatsesResource, c.ns, name, pt, data, subresources...), &v1alpha1.NodeDiskIOStats{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.NodeDiskIOStats), err
}
